...
module.exports = async(players = [], chId = null, sId)=>{
  try{
    const allyCodes = players.map(u=>u.allyCode)
    const rankCache = await mongo.find('rankCache', {allyCode: {$in: allyCodes}})
    const pSettings = await mongo.find('arena', {allyCode: {$in: allyCodes}})
